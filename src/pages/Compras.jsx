...
// ACTUALIZACIÓN: soporte para precarga desde Compra Sugerida (Fase 5)
// ============================================================
...
const navigate = useNavigate();
  ...
  // FASE 5 -- Compra Sugerida: si el modal se abrió precargado desde
  // una compra sugerida pendiente, guardamos su id acá para vincularla
  // una vez que la compra real se registre. null = alta normal, sin
  // ninguna relación con Compras Sugeridas.
  const [idCompraSugeridaPendiente, setIdCompraSugeridaPendiente] = useState(null);
...
useEffect(() => {
  if (!location.state?.idProveedor) return;

  setFiltroProveedor(String(location.state.idProveedor));
}, [location.state]);

  // FASE 5 -- Precarga desde una Compra Sugerida pendiente. Espera a
  // que "productos" ya esté cargado (lo necesita getVariantesDeProducto
  // para saber si cada línea requiere variante). No toca ninguna otra
  // lógica del modal: arma "lineas" e "idProveedor" con lo que vino de
  // /compras-sugeridas/:id/preparar-compra y abre el modal de siempre.
  useEffect(() => {
    if (!location.state?.precargaSugerida) return;
    if (productos.length === 0) return;

    const { idCompraSugerida, id_proveedor, productos: productosPrecarga } =
      location.state.precargaSugerida;

    setIdProveedor(String(id_proveedor));
    setLineas(
      productosPrecarga.map((p) => ({
    ...
    setIdCompraSugeridaPendiente(idCompraSugerida);
    setModalCompra(true);

    navigate(location.pathname, { replace: true, state: {} });
  }, [location.state, productos, navigate, location.pathname]);
...
const actualizarLinea = useCallback(
  ...
  const handleCrear = useCallback(async () => {
    ...
    try {
      ...
      // FASE 5 -- si esta compra vino precargada desde una Compra
      // Sugerida, la vinculamos y la marcamos como registrada. No
      // bloquea el flujo normal si falla: la compra real ya se creó
      // correctamente, esto es solo trazabilidad.
      if (idCompraSugeridaPendiente) {
        try {
          await axios.put(
            `${API_URL}/compras-sugeridas/${idCompraSugeridaPendiente}/registrar`,
            { id_compra: resCompra.data.compra.id_compra },
            { headers: getAuthHeader() },
          );
        } catch (errVinculo) {
          console.error("No se pudo vincular la compra sugerida:", errVinculo);
        }
      }
  ...
  const cerrarModalCompra = useCallback(() => {
    ...
    setErrorCrear("");
    setExitoCrear("");
    setIdCompraSugeridaPendiente(null);
  }, []);
...
useEffect(() => {
  if (!location.state?.abrirCompra) return;

  if (compras.length === 0) return;

  const compra = compras.find(
    (c) => Number(c.id_compra) === Number(location.state.abrirCompra)
  );

  if (!compra) return;

  const abrir = async () => {
    await abrirDetalle(compra);
  ...
  abrir();
}, [compras, location.state, abrirDetalle, navigate, location.pathname]);
            ...
            <div className="modal-body">
              {idCompraSugeridaPendiente && (
                <div className="alerta-info">
                  📋 Precargado desde una Compra Sugerida. Podés modificar
                  cantidades, precios, agregar o quitar productos antes de confirmar.
                </div>
              )}
              {exitoCrear && (
      ...
      <div className="modal-header">
        ...
        <div>
          ...
          <h2>
            ...
            Compra #{compraDetalle.id_compra}
          ...
          >
            ...
            <span>
              ...
              💳 {compraDetalle.forma_pago || "Sin especificar"}
        ...
        <div className="detalle-pago-section">
          ...
          <div className="pago-controles">
            ...
            <select
              ...
              onChange={(e) =>
                setNuevoEstadoPago(e.target.value)
              }
              disabled={!esEditable}
            >
            ...
            <select
              ...
              onChange={(e) =>
                setNuevaFormaPago(e.target.value)
              }
              disabled={!esEditable}
            >
              <option value="">
