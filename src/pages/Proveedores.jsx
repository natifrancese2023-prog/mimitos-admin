...
export default function Proveedores() {
  ...
  const handleGuardar = async () => {
    ...
    try {
      if (modoEdicion) {
        await axios.put(
          `${API_URL}/proveedores/${actual.id_proveedor}`,
          actual,
          { headers: getAuthHeader() },
        );
        setMensajeExito("Proveedor actualizado correctamente");
      } else {
        await axios.post(`${API_URL}/proveedores`, actual, {
          headers: getAuthHeader(),
        });
      ...
      setErroresForm({
        general: err.response?.data?.error || "Error al guardar",
      });
    } finally {
      setGuardando(false);
    }
  ...
  const handleEliminar = async () => {
    if (!confirmar) return;
    try {
      await axios.delete(`${API_URL}/proveedores/${confirmar.id_proveedor}`, {
        headers: getAuthHeader(),
      });
      setConfirmar(null);
      await cargarProveedores();
  ...
  const abrirHistorial = async (p) => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/proveedores/${p.id_proveedor}/compras`,
        { headers: getAuthHeader() },
      );
  ...
  const formatFecha = (f) =>
    f
      ...
      : "—";
  const totalCompras = (h) =>
    h.reduce((acc, c) => acc + Number(c.total || 0), 0);

  // ── Renders estado ─────────────────────────────────────
  if (cargando)
    return (
      <div className="prov-estado">
  ...
  if (error)
    return (
      <div className="prov-estado prov-error">
        <span>⚠️</span>
        <p>{error}</p>
  ...
  return (
    <div className="proveedores-page">
      ...
      {/* TABLA */}
      {filtrados.length === 0 ? (
        <div className="prov-vacio">
          <span>🏭</span>
          <p>
            {busqueda ? "No hay resultados" : "No hay proveedores registrados"}
          </p>
        </div>
      ) : (
            ...
            <tbody>
              {filtrados.map((p) => (
                <tr key={p.id_proveedor}>
                  <td>
                    <span className="prov-nombre">{p.nombre}</span>
                  </td>
                  <td>{p.telefono || "—"}</td>
                  <td>{p.direccion || "—"}</td>
                  <td>
                    <div className="acciones">
                      <button
                        className="btn-ver"
                        onClick={() =>
                          navigate(`/panel/proveedores/${p.id_proveedor}`)
                        }
                      >
                        👁 Detalle
                      </button>
        ...
        <div className="modal-overlay" onClick={cerrarABM}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              ...
              <button className="modal-cerrar" onClick={cerrarABM}>
                ✕
              </button>
            </div>
              ...
              <div className="form-col">
                ...
                <div className="field-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setActual({ ...actual, telefono: e.target.value })
                    }
                  />
                </div>
                <div className="field-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setActual({ ...actual, direccion: e.target.value })
                    }
                  />
                </div>
              </div>
            ...
            <div className="modal-footer">
              <button className="btn-secundario" onClick={cerrarABM}>
                Cancelar
              </button>
              <button
        ...
        <div className="modal-overlay" onClick={cerrarHistorial}>
          ...
          >
            <div className="modal-header">
              ...
              <button className="modal-cerrar" onClick={cerrarHistorial}>
                ✕
              </button>
            </div>
            <div className="modal-body">
                  ...
                  <div className="hist-resumen">
                    ...
                    <div className="resumen-item">
                      <span className="resumen-valor">
                        ${totalCompras(historial).toLocaleString("es-AR")}
                      </span>
                    ...
                    <div className="resumen-item">
                      <span className="resumen-valor">
                        {
                          historial.filter((c) => c.estado_pago === "pendiente")
                            .length
                        }
                      </span>
          ...
          >
            ...
            <div className="modal-body">
              <p>
                ...
                <strong>{confirmar.nombre}</strong>? Esta acción no se puede
                deshacer.
              </p>
            </div>
            <div className="modal-footer">
